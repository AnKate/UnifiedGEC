...
fairseq_dir = os.path.abspath(os.path.join(os.path.dirname(__file__),'..'))
    ...
    def make_all_matrix(info_format):  # 二进制化所有使用numpy矩阵存储的数据，主要是句法相关的掩码矩阵、距离矩阵和概率矩阵
        ...
        if args.trainpref:
            for suffix in suffixs:
                train_src_file = args.trainpref.replace("bpe", suffix) + ".src"     # 英文
                # train_src_file = args.trainpref.replace("char", suffix) + ".src"  # 中文
        ...
        if args.validpref:
            for suffix in suffixs:
                ...
                # valid_src_file = args.validpref.replace("char", suffix) + ".src"  # 中文
        ...
        if args.testpref:
            for suffix in suffixs:
                test_src_file = args.testpref.replace("bpe", suffix) + ".src" # 英文
                # test_src_file = args.testpref.replace("char", suffix) + ".src" # 中文
...
def cli_main():
    ...
    args.dataset_impl = args.data_impl # 3.28新增
    main(args)
