...
class LanguageModelingConfig(FairseqDataclass):
    ...
    tokens_per_sample: int = field(
        default=1024,
        metadata={"help": "max number of tokens per sample for LM data"},
    )
    ...
    shorten_data_split_list: str = field(
        default="",
        metadata={
            "help": "comma-separated list of data splits to apply shortening to, "
            'e.g., "train,valid" (default: all data splits)'
        },
    )
    # TODO common vars below add to parent
    seed: int = II("params.common.seed")
    dataset_impl: Optional[ChoiceEnum(get_available_dataset_impl())] = II(
        "params.data.dataset_impl"
    )
    data_buffer_size: int = II("params.data.data_buffer_size")
