...
def post_process_prediction(
    ...
    hypo_str = tgt_dict.string( # 将预测的tokens转化成中文（空格分开）
        hypo_tokens, remove_bpe, extra_symbols_to_ignore=extra_symbols_to_ignore
    )
    if align_dict is not None: # 跳过
        hypo_str = replace_unk(
            hypo_str, src_str, alignment, align_dict, tgt_dict.unk_string()
        )
    if align_dict is not None or remove_bpe is not None:
        # Convert back to tokens for evaluating with unk replacement or without BPE
        # Note that the dictionary can be modified inside the method.  如果存在对齐字典或者需要移除 BPE 标记，那么最终将生成的字符串再次转换回标记形式。
...
def import_user_module(args):
    module_path = getattr(args, "user_dir", None) # 从 args 中获取 user_dir 属性，即用户指定的模块路径，如果不存在，则为 None
    if module_path is not None:# 如果 module_path 存在。 '../../src/src_syngec/syngec_model'
        module_path = os.path.abspath(args.user_dir)   # 获取绝对路径。
        if not os.path.exists(module_path):  # 尝试通过相对路径拼接 Fairseq 源代码目录下的路径进行查找。
        ...
        # ensure that user modules are only imported once 具有 memo 属性，用于记录已经导入过的模块。
        import_user_module.memo = getattr(import_user_module, "memo", set())
        if module_path not in import_user_module.memo: # 如果模块路径不在 memo 中。
            import_user_module.memo.add(module_path) # 将模块路径添加到 memo 中，表示已经导入过。
            # 提取模块路径的父目录和模块名
            module_parent, module_name = os.path.split(module_path)
            if module_name not in sys.modules:  # 如果模块名不在 sys.modules 中，将父目录插入到 Python 搜索路径的开头，然后通过 importlib.import_module 动态导入模块。
