...
def load_checkpoint(args, trainer, **passthrough_args):
    ...
    *passthrough_args* will be passed through to
    ...
    if getattr(args, "finetune_from_model", None) is not None and (
        ...
        raise ValueError( # 如果启用了微调模型（finetune_from_model 不为 None），并且设置了重置优化器、学习率调度器、重置指标、重置数据加载器中的任何一个，则抛出异常
            "--finetune-from-model can not be set together with either --reset-optimizer"
            " or reset_lr_scheduler or reset_meters or reset_dataloader"
        )

    suffix = getattr(args, "checkpoint_suffix", "")  # 获取检查点文件名后缀，如果未设置，则为空字符串。
    if (
        args.restore_file == "checkpoint_last.pt"  # 如果 restore_file 的值为 "checkpoint_last.pt"（默认值），则构建最后一个检查点文件的路径。
    ):  # default value of restore_file is 'checkpoint_last.pt'
        checkpoint_path = os.path.join(
            args.save_dir, "checkpoint_last{}.pt".format(suffix)
        )
        first_launch = not PathManager.exists(checkpoint_path)
        if getattr(args, "finetune_from_model", None) is not None and first_launch: # 如果设置了 finetune_from_model 并且是第一次启动，则从预训练模型开始微调。
            # if there is no last checkpoint to restore, start the finetune from pretrained model
            # else just use usual logic to load checkpoint, e.g. restart from last checkpoint and etc.

            file_path = os.path.abspath(os.path.join(os.getcwd(), args.finetune_from_model))
            # print('当前路径：',os.getcwd())
            # print('file_path(args.finetune_from_model):',file_path)
    ...
    elif getattr(args, "model_parallel_size", 1) > 1: # 如果模型并行数大于 1，则构建检查点文件路径，将 ".pt" 替换为后缀（suffix）。
        checkpoint_path = args.restore_file.replace(".pt", suffix + ".pt")
    else: # 否则，使用指定的 restore_file。
        checkpoint_path = args.restore_file

    if args.restore_file != "checkpoint_last.pt" and getattr(
        args, "finetune_from_model", None
    ):
        raise ValueError( # 如果 restore_file 不是 "checkpoint_last.pt"，并且设置了 finetune_from_model，则抛出异常，因为这两者不能同时指定
    ...
    trainer.lr_step(epoch_itr.epoch) # 根据当前 epoch 更新学习率。这通常用于学习率调度器在每个 epoch 结束时进行更新。
...
def verify_checkpoint_directory(save_dir: str) -> None:
    ...
    temp_file_path = os.path.join(save_dir, "dummy")  # # 在目录中创建一个临时文件（"dummy"），以检查是否有写入权限
    try:  # 如果无法创建或写入临时文件，则发出警告
        ...
        os.remove(temp_file_path)  # 如果成功创建和写入临时文件，则删除该文件
