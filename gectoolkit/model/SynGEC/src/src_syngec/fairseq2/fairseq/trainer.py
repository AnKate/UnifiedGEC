...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, quantizer=None):
        ...
        self._criterion = criterion    # 导入评估器
        self._model = model   # 导入模型
        if self.tpu:
            import torch_xla.core.xla_model as xm

            self._model = xm.send_cpu_data_to_device(self._model, self.device)
        if args.fp16:  # True 半精度是指使用16位浮点数来表示模型参数和梯度，以减少内存占用和加速训练过程。通常，这被称为混合精度训练
            self._criterion = self._criterion.half()  # 损失函数（self._criterion）的数据类型转换为半精度（half precision）。
            self._model = self._model.half()  # 模型参数的数据类型转换为半精度
        elif args.bf16:  # 如果启用了 bfloat16 训练（bf16 为真），则将评估器和模型参数的数据类型转换为 bfloat16。
            self._criterion = self._criterion.to(dtype=torch.bfloat16)
            self._model = self._model.to(dtype=torch.bfloat16)
        if not args.pipeline_model_parallel: # 如果没有启用管道模型并行化（pipeline_model_parallel 为假），则将评估器和模型参数移动到当前设备。
        ...
        if self.cuda:
            self.cuda_env = utils.CudaEnvironment() # 如果启用了 CUDA，创建一个 CudaEnvironment 对象，该对象用于获取和存储 CUDA 环境的详细信息。
            if self.data_parallel_world_size > 1: # 如果使用了数据并行（data parallelism），则使用 distributed_utils.all_gather_list 函数将每个 GPU 上的 CUDA 环境信息进行收集，存储在 cuda_env_arr 中。
                self.cuda_env_arr = distributed_utils.all_gather_list(self.cuda_env)
            else: # 如果没有使用数据并行，则将单个 CUDA 环境信息存储在列表中。
                self.cuda_env_arr = [self.cuda_env]
            if self.data_parallel_rank == 0: # 如果是数据并行中的主 GPU（rank为0），则使用 pretty_print_cuda_env_list 方法打印所有 GPU 的 CUDA 环境信息。
                utils.CudaEnvironment.pretty_print_cuda_env_list(self.cuda_env_arr)
        else: # 如果没有启用 CUDA，将 cuda_env 和 cuda_env_arr 设置为 None。
            self.cuda_env = None
            self.cuda_env_arr = None
        # 使用 metrics.log_start_time 记录当前时间，标记为 "wall"，设置优先级为 790，回合为 0。这通常用于记录训练的时间信息。
    ...
    def _build_optimizer(self):
        ...
        if self.args.fp16 or self.args.bf16:
            ...
            if self.args.memory_efficient_fp16 or self.args.memory_efficient_bf16:
                ...
                self._optimizer = optim.FP16Optimizer.build_optimizer(self.args, params) # 运行
        else:
            if self.cuda and torch.cuda.get_device_capability(0)[0] >= 7:
                logger.info("NOTE: your device may support faster training with --fp16")
            self._optimizer = optim.build_optimizer(self.args, params)

        if self.args.use_bmuf:  # 跳过
    ...
    def load_checkpoint(
        ...
        if getattr(self.args, "syntax_model_file", None) is not None:
            ...
            )# 如果存在语法模型文件路径（syntax_model_file），则加载该语法模型的状态字典到 CPU，并使用模型的 load_syntax_state_dict 方法加载状态。
        ...
        if getattr(self.args, "bart_model_file_from_transformers", None) is not None:
            ...
            )# 如果存在 BART 模型文件路径（bart_model_file_from_transformers），则根据路径加载预训练的 BART 模型，并使用模型的 load_bart_state_dict_from_transformers 方法加载状态。

        bexists = PathManager.isfile(filename) # 检查指定的检查点文件是否存在。
        if bexists:
            state = checkpoint_utils.load_checkpoint_to_cpu(filename)

            # load model parameters 加载模型的状态字典。如果模型的参数不匹配，会引发异常。
            try:
                ...
                if utils.has_parameters(self.get_criterion()):
                    ...
                    ) # 如果损失函数（criterion）具有参数，则加载损失函数的状态字典。
            except Exception:
                raise Exception(
                    "Cannot load model parameters from checkpoint {}; "
                    "please ensure that the architectures match.".format(filename)
                )
            # 从加载的状态中获取额外状态、优化器历史记录和最后一个优化器状态。
            extra_state = state["extra_state"]
            self._optim_history = state["optimizer_history"]
            last_optim_state = state.get("last_optimizer_state", None)
        # 如果存在最后一个优化器状态且未设置重置优化器，则重建优化器。
        if last_optim_state is not None and not reset_optimizer:
            ...
            if not reset_lr_scheduler: # 如果没有设置重置学习率调度器，则加载最后一个优化器的学习率调度器状态字典。
    ...
    def get_train_iterator(
        ...
        batch_iterator = self.task.get_batch_iterator(
            ...
            max_tokens=self.args.max_tokens,  #  每个批次的最大tokens
            max_sentences=self.args.batch_size,  # 每个批次的最大句子数 todo 控制
            max_positions=utils.resolve_max_positions( # 模型支持的最大句子长度
            ...
            ignore_invalid_inputs=True, # 是否忽略那些长度过长的句子，而不引发异常。默认值为 False，表示不忽略。
            required_batch_size_multiple=self.args.required_batch_size_multiple, # todo 控制 要求批次大小是某个数目的倍数。默认值为 1
            seed=self.args.seed,   # 用于生成随机数的种子，以确保可重复性。默认值为 1
            num_shards=self.data_parallel_world_size if shard_batch_itr else 1, # 数据迭代器的分片数目
            shard_id=self.data_parallel_rank if shard_batch_itr else 0, # 要返回的数据迭代器的分片编号
            num_workers=self.args.num_workers,  # 用于数据加载的子进程数。如果为 0，数据将在主进程中加载。默认值为 0
            epoch=epoch,  # 要从哪个时期开始迭代器。默认值为 1
            data_buffer_size=self.args.data_buffer_size, # 预加载的批次数目
            disable_iterator_cache=disable_iterator_cache, # 是否禁用 EpochBatchIterator 的缓存。如果启用缓存，会忽略 FairseqTask::can_reuse_epoch_itr。默认值为 False
            type='train'  # 判断是否shuffle
        )
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                with maybe_no_sync():
                    ...
                    print('----- learning rate:',self.optimizer.get_lr())
                    del loss
        ...
        if hasattr(self.model, "all_reduce"): # 跳过  分布式设置中对模型参数进行全局归约操
            self.model.all_reduce()

        overflow = False
        try:
            if self.tpu and self.data_parallel_world_size > 1: # 跳过
                import torch_xla.core.xla_model as xm
            ...
            if not self.tpu:
                if (
                    not self.args.use_bmuf  # BMUF 会保持每个 GPU 的动量在字节级别上对齐，
                    and self.args.distributed_wrapper != "SlowMo"
                ):
                    self._check_grad_norms(grad_norm) # 运行了
                    print('---- 运行了self._check_grad_norms(grad_norm)')
        ...
        # Some distributed wrappers (e.g., SlowMo) need access to the optimizer after the step  (overflow detected)
        ...
        if not overflow or self.args.distributed_wrapper == "SlowMo":
            ...
            if self.tpu:
                ...
                print('----- 运行了self._reduce_and_log_stats：')
    ...
    def _reduce_and_log_stats(self, logging_outputs, sample_size, grad_norm=None):
        ...
        with metrics.aggregate() as agg:
            if logging_outputs is not None:
                ...
                # net_output = logging_outputs[0]['net_output'][0]  # 5.6新增记录net_output
                del logging_outputs
            ...
            if self.tpu:
                ...
                # logging_output['net_output'] = net_output # 5.6新增net_output
