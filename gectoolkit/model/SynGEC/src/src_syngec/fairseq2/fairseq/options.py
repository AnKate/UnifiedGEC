...
def get_training_parser(default_task="translation"):
    ...
    add_dataset_args(parser, train=True) # 该函数向解析器对象 parser 中添加与数据集相关的命令行参数
    add_distributed_training_args(parser)  # 该函数向解析器对象 parser 中添加与分布式训练相关的命令行参数
    add_model_args(parser) # 该函数向解析器对象 parser 中添加与模型相关的命令行参数
...
def add_preprocess_args(parser):
    ...
    parser.add_argument('--data-impl', metavar='FORMAT', default='mmap',
                        choices=get_available_dataset_impl(),
                        help='output data implementation')
