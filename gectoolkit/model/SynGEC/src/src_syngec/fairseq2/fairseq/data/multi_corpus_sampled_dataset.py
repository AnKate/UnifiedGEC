...
class MultiCorpusSampledDataset(FairseqDataset):
    ...
    creates a batch by first sampling a data according to a specified
    probability distribution and then getting instances from that data.

    Args:
        datasets: an OrderedDict of FairseqDataset instances.
        sampling_func: A function for sampling over list of data keys.
    ...
    def __len__(self):
        """
        Length of this data is the sum of individual datasets
        """
    ...
    def ordered_indices(self):
        ...
        data's ordered_indices() so that we get the same random ordering
        as we would have from using the underlying data directly.
        """
    ...
    def _map_index_to_dataset(self, key: int, index: int):
        ...
        we are not accessing an index outside the range of the current data
    ...
    def __getitem__(self, index: int):
        """
        Get the item associated with index from each underlying data.
        Since index is in the range of [0, TotalNumInstances], we need to
        map the index to the data before retrieving the item.
        """
        return OrderedDict(
            [
    ...
    def collater(self, samples: List[Dict]):
        """
        Generate a mini-batch for this data.
        To convert this into a regular mini-batch we use the following
        logic:
            1. Select a data using the specified probability distribution.
            2. Call the collater function of the selected data.
        """
        if len(samples) == 0:
            return None
