...
def default_virtual_size_func(datasets, ratios, max_scale_up=1.5):
    ...
    # set virtual sizes relative to the largest data
...
class SampledMultiDataset(FairseqDataset):
    ...
    Args:
        ...
        sampling_ratios (List[float]): list of probability of each data to be sampled
            (default: None, which corresponds to concatenating all data together).
        ...
        virtual_size (int, or callable): the expected virtual size of the data (default: default_virtual_size_func).
    ...
    def __init__(
        ...
        # self.datasets[k][self._cur_indices[i]] is the data item i in this sampled data
        # namely, data item i is sampled from the kth sub-data self.datasets[k]
    ...
    def get_virtual_indices(self, rng, datasets, sample_ratios, virtual_size):
        ...
        def get_in_dataset_indices(datasets, sizes, sample_ratios):
            ...
            # uniformally sample desired counts for each data
    ...
    def _establish_virtual_datasets(self):
        if self.sample_ratios is None and self._cur_indices is not None:
            # not a samping data, no need to resample if indices are already established
            return
        self._reset_cached_properties()
        ...
        if self.sample_ratios is not None:
            ...
            logger.info(f"[{self.split}] A concat data")
        logger.info(
            f"[{self.split}] virtual data established time: {get_time_gap(start_time, time.time())}"
        )
