...
class ResamplingDataset(BaseWrapperDataset):
    """Randomly samples from a given data at each epoch.
    ...
    to increase per-epoch coverage of the base data (since sampling with
    replacement means that many items in the data will be left out). In the
    case of sampling without replacement, size_ratio should be strictly less
    than 1.

    Args:
        dataset (~torch.utils.data.Dataset): data on which to sample.
