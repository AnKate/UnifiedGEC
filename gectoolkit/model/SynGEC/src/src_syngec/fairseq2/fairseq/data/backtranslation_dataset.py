...
class BacktranslationDataset(FairseqDataset):
    """
    Sets up a backtranslation data which takes a tgt batch, generates
    a src using a tgt-src backtranslation function (*backtranslation_fn*),
    and returns the corresponding `{generated src, input tgt}` batch.

    Args:
        tgt_dataset (~fairseq.data.FairseqDataset): the data to be
            backtranslated. Only the source side of this data will be used.
            After backtranslation, the source sentences in this data will be
    ...
    def num_tokens(self, index):
        """Just use the tgt data num_tokens"""
        return self.tgt_dataset.num_tokens(index)

    def ordered_indices(self):
        """Just use the tgt data ordered_indices"""
        return self.tgt_dataset.ordered_indices()

    def size(self, index):
        """Return an example's size as a float or tuple. This value is used
        when filtering a data with ``--max-positions``.
