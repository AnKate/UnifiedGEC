    ...
    def next_epoch_itr(self, shuffle=True, fix_batches_to_gpus=False):
        ...
        Args:
            ...
            fix_batches_to_gpus: ensure that batches are always
                ...
                that :attr:`data` supports prefetching (default: False).
        """
        raise NotImplementedError
...
class EpochBatchIterator(EpochBatchIterating):
    ...
    Args:
        dataset (~torch.utils.data.Dataset): data from which to load the data
    ...
    def first_batch(self):
        if len(self.frozen_batches) == 0:
            raise Exception(
                "The data is empty. This could indicate "
                "that all elements in the data have been skipped. "
                "Try increasing the max number of allowed tokens or using "
                "a larger data."
            )
        # 检查数据集是否支持在数据加载器之外进行数据提取。
        if getattr(self.dataset, "supports_fetch_outside_dataloader", True):
            return self.collate_fn([self.dataset[i] for i in self.frozen_batches[0]]) # 使用 self.frozen_batches[0] 中的索引从数据集中提取数据，并调用 self.collate_fn 函数对这些数据进行批次整理（collation）。
        else:
            return "DUMMY"
    ...
    def next_epoch_idx(self):
        ...
        if self._next_epoch_itr is not None:
            ...
            return self.epoch + 1  # 当前 epoch 已经结束（self.end_of_epoch() 返回 True），则返回当前 epoch 索引加 1
        else:
            return self.epoch
    ...
    def next_epoch_itr(self, shuffle=True, fix_batches_to_gpus=False):
        ...
        self.epoch = self.next_epoch_idx
        if hasattr(self.dataset, "set_epoch"):
            self.dataset.set_epoch(self.epoch) # 具有 set_epoch 方法，则调用该方法并传递当前 epoch 的索引。
        if self._next_epoch_itr is not None:
            self._cur_epoch_itr = self._next_epoch_itr
            self._next_epoch_itr = None # 表示下一个 epoch 的迭代器已经使用过
        else:
