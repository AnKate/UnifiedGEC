...
class SampledMultiEpochDataset(SampledMultiDataset):
    ...
    Args:
        ...
        sampling_ratios (List[float]): list of probability of each data to be sampled
            (default: None, which corresponds to concating all data together).
        ...
        virtual_epoch_size (int): virtual epoch size, the data will go through the data by
            this virtual epoch size one by one to speed up data loading, e.g. indicing and filtering
            can be performed whenever a virtual epoch is loaded without waiting for the whole data to be loaded.
    ...
    def _setup(self, epoch):
        ...
        if self.virtual_epoch_size > self.virtual_size:
            logger.warning(
                ...
                f"is greater than virtual data size {self.virtual_size}"
            )
        ...
        logger.info(
            f"virtual epoch size {self.virtual_epoch_size}; virtual data size {self.virtual_size}"
        )
    ...
    def set_epoch(self, epoch):
        if self._current_epoch_start_index is None:
            # initializing epoch idnices of a virtual data
            self._setup(epoch)
