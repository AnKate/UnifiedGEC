...
class BlockPairDataset(FairseqDataset):
    ...
    Args:
        dataset (~torch.utils.data.Dataset): data to break into blocks
    ...
    def _sent_to_dataset_index(self, sent_sizes):
        """
        Build index mapping block indices to the underlying data indices
        """
        dataset_index = []
        ...
        for to_consume in sent_sizes:
            ...
            dataset_index.append(
                (
                    start_ds_idx,  # starting index in data
                    start_offset,  # starting offset within starting index
                    ds_idx,  # ending index in data
    ...
    def _truncate_sentences(self, sent_a, sent_b, max_num_tokens):
        ...
        Returns:
            Truncated sentences represented by data idx
        """
    ...
    def _cut_sentence(self, sent, front_cut, end_cut):
        ...
        Represent the sentence as data idx and return
        """
    ...
    def _fetch_block(self, start_ds_idx, offset, end_ds_idx, length):
        """
        Fetch a block of tokens based on its data idx
        """
        buffer = torch.cat(
