...
class MultiCorpusDataset(FairseqDataset):
    ...
    to be the same data, as the collate method needs to work on batches with
    samples from each data.
    ...
    def ordered_indices(self):
        with data_utils.numpy_seed(self.seed, self.epoch):
            # Used to store the order of indices of each data to use
            indices = [
                np.random.permutation(len(dataset))
                for dataset in self.datasets.values()
            ]
            # Keep track of which samples we've  used for each data
    ...
    def _map_index(self, index: int):
        """
        If data A has length N and data B has length M
        then index 1 maps to index 1 of data A, and index N + 1
