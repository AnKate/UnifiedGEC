...
class LevenshteinTransformer(nn.Module):
    def __init__(self, config, dataset):
        ...
        if self.language == "zh":
            self.checkpoint_dir = config["pretrained_model_path"] + '/Chinese'
        elif self.language == "en":
            self.checkpoint_dir = config["pretrained_model_path"] + '/English'
        else:
            self.checkpoint_dir = config["pretrained_model_path"] + '/Multilingual'
    ...
    def initial_parameters_bp(self):
        checkpoint_dir = self.checkpoint_dir
        print([k[0] for k in self.encoder.named_parameters()])
        pretrained_model = torch.load(os.path.join(checkpoint_dir, 'pytorch_model.bin'))
        new_pretrained_model = {}
        for k in pretrained_model.keys():
            v = pretrained_model[k]
            if k == 'bert.embeddings.word_embeddings.weight':
                new_pretrained_model['tok_embed.weight'] = torch.cat([v, v[:11]], 0)
            elif k == 'bert.embeddings.position_embeddings.weight':
                new_pretrained_model['pos_embed.weights.weight'] = torch.cat([v, v], 0)
            elif k == 'bert.embeddings.token_type_embeddings.weight':
                new_pretrained_model['seg_embed.weight'] = v
            elif re.match('bert.encoder.layer.[0-9].attention.output.dense.weight', k):
                num= k.split('.')[3]
                new_pretrained_model['layers.%s.self_attn.out_proj.weight' %num] = v
            elif re.match('bert.encoder.layer.[0-9].attention.output.dense.bias', k):
                num= k.split('.')[3]
                new_pretrained_model['layers.%s.self_attn.out_proj.bias' %num] = v
            elif re.match('bert.encoder.layer.[0-9].intermediate.dense.weight', k):
                num= k.split('.')[3]
                new_pretrained_model['layers.%s.fc1.weight' %num] = v
            elif re.match('bert.encoder.layer.[0-9].intermediate.dense.bias', k):
                num= k.split('.')[3]
                new_pretrained_model['layers.%s.fc1.bias' %num] = v
            elif re.match('bert.encoder.layer.[0-9].output.dense.weight', k):
                num= k.split('.')[3]
                new_pretrained_model['layers.%s.fc2.weight' %num] = v
            elif re.match('bert.encoder.layer.[0-9].output.dense.bias', k):
                num= k.split('.')[3]
                new_pretrained_model['layers.%s.fc2.bias' %num] = v
            elif re.match('bert.encoder.layer.[0-9].attention.self.query.weight', k):
                num= k.split('.')[3]
                key = pretrained_model['bert.encoder.layer.%s.attention.self.key.weight' %num]
                value = pretrained_model['bert.encoder.layer.%s.attention.self.value.weight' % num]
                new_pretrained_model['layers.%s.self_attn.in_proj_weight' %num] = torch.cat([v, key, value], 0)
            elif re.match('bert.encoder.layer.[0-9].attention.self.query.bias', k):
                num= k.split('.')[3]
                key = pretrained_model['bert.encoder.layer.%s.attention.self.key.bias' %num]
                value = pretrained_model['bert.encoder.layer.%s.attention.self.value.bias' % num]
                new_pretrained_model['layers.%s.self_attn.in_proj_bias' %num] = torch.cat([v, key, value], 0)
            else:
        ...
        torch.save(self.encoder.state_dict(), os.path.join(checkpoint_dir, 'pre-train.ckpt')); exit()
        # print(pretrained_model['model'].keys()); exit()

    def initial_parameters(self):
        checkpoint_dir = self.checkpoint_dir
        pretrained_model = torch.load(os.path.join(checkpoint_dir, 'pre-train.ckpt'))
        # print(pretrained_model); exit()
        # new_pretrained_model = {}
        # for k in pretrained_model.keys():
        #     v = pretrained_model[k]
        #     if k == 'tok_embed.weight':
        #         new_pretrained_model[k] = v[:1]
    ...
    def forward(self, batch, dataloader, is_display=False):
        ...
        if is_display:
            print('word_del_pred', word_del_pred[0], word_del_targets[0])
        ...
        if is_display:
            print('mask_ins_pred', mask_ins_pred[0], masked_ins_targets[0]); # exit()
        ...
        if is_display:
            print(self.pad_id)
            print('self.decode_result', self.decode_result[0], tgt_tokens[0])
        # exit()
    ...
    def model_test(self, batch, dataloader):
        ...
        for _ in range(1):
            ...
            word_del_targets = _get_del_targets(src_tokens, tgt_tokens, self.pad_id)
            #print('word_del_pred', src_tokens[0], word_del_targets[0])

            output_tokens, _, _, _, _ = _apply_del_words(
                src_tokens, None, None, word_del_pred.bool(), # word_del_targets
                self.pad_id, self.bos_id, self.eos_id
            )
            ...
            masked_tgt_masks, masked_tgt_tokens, masked_ins_targets = _get_ins_targets(
                output_tokens, tgt_tokens, self.pad_id, self.unk_id
            )
            #print('word_del_pred', output_tokens[0], masked_ins_targets[0])
            output_tokens, _, _, _ = _apply_ins_masks(
                output_tokens, None, mask_ins_pred, #,masked_ins_targets
            ...
            #print('output_tokens', output_tokens[0], tgt_tokens[0])
            output_tokens, _ = _apply_ins_words(
                output_tokens, None, word_ins_pred[:, :output_tokens.size(1)], #tgt_tokens
                None, self.unk_id
            )
