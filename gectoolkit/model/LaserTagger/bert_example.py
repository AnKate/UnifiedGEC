...
class BertExample(object):
    ...
    def to_torch_example(self):
        """Returns this object as a PyTorch example."""
        torch_features = {
            key: torch.tensor(val, dtype=torch.long) for key, val in self.features.items()
        }
...
class BertExampleBuilder(object):
    ...
    def __init__(self, label_map, vocab_file,
        ...
        self._tokenizer = BertTokenizer.from_pretrained(vocab_file, do_lower_case=do_lower_case)
    ...
    def build_bert_example(
        ...
        tokens, labels, token_start_indices = self._split_to_wordpieces(
            # wordpiece： tag是以ｗｏｒｄ为单位的，组成ｗｏｒｄ的ｐｉｅｃｅ的标注与这个ｗｏｒｄ相同
    ...
    def _split_to_wordpieces(self, tokens, labels):
        ...
        for i, token in enumerate(tokens):
            ...
            # pieces = self._tokenizer.encode(token).tokens
