...
def get_sari(source_ids, prediction_ids, target_ids, max_gram_size=4):
    ...
    Args:
        source_ids: A 2D torch.Tensor of size (batch_size, sequence_length)
        prediction_ids: A 2D torch.Tensor of size (batch_size, sequence_length)
        target_ids: A 3D torch.Tensor of size (batch_size, number_of_targets, sequence_length)
        max_gram_size: int. largest n-gram size we care about (e.g. 3 for unigrams,
            bigrams, and trigrams)
    ...
    source_ids = source_ids.numpy()
    prediction_ids = prediction_ids.numpy()
    target_ids = target_ids.numpy()

    sari_scores, keep_scores, add_scores, deletion_scores = get_sari_numpy(source_ids, prediction_ids, target_ids, max_gram_size)

    sari_scores = torch.tensor(sari_scores, dtype=torch.float32)
    keep_scores = torch.tensor(keep_scores, dtype=torch.float32)
    add_scores = torch.tensor(add_scores, dtype=torch.float32)
    deletion_scores = torch.tensor(deletion_scores, dtype=torch.float32)

    return sari_scores, keep_scores, add_scores, deletion_scores
...
def sari_score(predictions, labels, features, **unused_kwargs):
    ...
    if "inputs" not in features:
        raise ValueError("sari_score_pytorch requires inputs feature")

    # Convert the inputs and outputs to a [batch_size, sequence_length] tensor.
    inputs = torch.squeeze(features["inputs"], dim=-1)
