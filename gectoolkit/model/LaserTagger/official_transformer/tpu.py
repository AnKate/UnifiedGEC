...
#     """
#     with torch.name_scope(name):
#         n_embeddings = embedding_table.shape[0]
#         batch_size, padded_size = values.shape
#
#         emb_idcs = values.view(batch_size, padded_size, 1).repeat(1, 1, n_embeddings)
#         emb_weights = mask.view(batch_size, padded_size, 1).repeat(1, 1, n_embeddings)
#         col_idcs = torch.arange(n_embeddings).view(1, 1, n_embeddings).repeat(batch_size, padded_size, 1)
#         one_hot = torch.where(
#             emb_idcs == col_idcs, emb_weights,
#             torch.zeros(batch_size, padded_size, n_embeddings, dtype=torch.float32))
#
#         return torch.einsum("ijk,kl->ijl", one_hot, embedding_table)
