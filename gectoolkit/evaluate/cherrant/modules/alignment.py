...
# from scorers.ChERRANT.modules.tokenizer import Tokenizer # 7.30改
from .tokenizer import  Tokenizer
# from modules.tokenizer import Tokenizer # 原本的
import os
...
class Alignment:
    ...
    def _dfs(self, i, j, align_seq_now, oper_matrix, strategy="all"):
        ...
        """
        if len(align_seq_now) > 100:
            # print("Recursion max")
            return False
        if i + j == 0:
            self.align_seqs.append(align_seq_now)
            # print('self.align_seqs', self.align_seqs)
        else:
