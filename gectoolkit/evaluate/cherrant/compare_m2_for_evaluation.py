...
def main(args):
    ...
    for sent_id, sent in enumerate(sents):
        ...
        if  args.reference_num is None or len(ref_dict.keys()) == args.reference_num:
            ...
            best_dict += Counter(count_dict)               # Counter({'tp': 1, 'fn': 1})
            best_cats = merge_dict(best_cats, cat_dict)    # {'S': [1, 0, 1]}
    # Print results
    TP,FP,FN,Prec,Rec,F = print_results(best_dict, best_cats, args)
    # print('best_dict:',best_dict)
    # print('best_cats:',best_cats)
    # print('args:',args)

    return TP,FP,FN,Prec,Rec,F

    # print(sent_id_cons)
    # print(len(sent_id_cons))

class Args:
    def __init__(self, hyp, ref, start=None, end=None, max_answer_num=None, reference_num=None, beta=0.5, verbose=False,
                 dt=False, ds=False, cs=True, cse=False, single=False, multi=False, multi_hyp_avg=False, multi_hyp_max=False,
                 filt=None, cat=None):
        self.hyp = hyp
        self.ref = ref
        self.start = start
        self.end = end
        self.max_answer_num = max_answer_num
        ...
        self.single = single
        self.multi = multi
        self.multi_hyp_avg = multi_hyp_avg
        self.multi_hyp_max = multi_hyp_max
...
def simplify_edits(sent, max_answer_num):
    out_edits = []
    # Get the edit lines from an m2 block. ['S ..','T0 ..','A ..']
    edits = sent.split("\n")
...
def process_edits(edits, args):
    ...
    for edit in edits:
        ...
        # 1. 过滤掉类别为"UNK"（未知类别）的编辑。 UNK type edits are only useful for detection, not correction.
...
def print_results(best, best_cats, args):
    ...
    PRF = list(computeFScore(best["tp"], best["fp"], best["fn"], args.beta))
    Prec = PRF[0]
    Rec = PRF[1]
    F = PRF[2]
    TP = best["tp"]
    ...
    # print('{:=^46}'.format(""))
    # 
    # print("best[tp]:",best["tp"])
    # print("best[fp]:", best["fp"])
    # print("best[fn]:", best["fn"])
    # print(list(computeFScore(best["tp"], best["fp"], best["fn"], args.beta)))
    # print(computeFScore(best["tp"], best["fp"], best["fn"], args.beta))
    # exit()
