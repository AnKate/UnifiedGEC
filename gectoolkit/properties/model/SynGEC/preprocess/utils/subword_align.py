...
def align(sent_tuple):
    ...
    if len(aligned_results) != len(tgt_tokens) or int(aligned_results[-1]) != (len(src_tokens) - 1):
        print('------------------utils/subword_align.py-------------')
        print('len(aligned_results):', len(aligned_results))
        print('len(tgt_tokens):', len(tgt_tokens))

        print('aligned_results:', aligned_results[-1])
        print('len(src_tokens)-1:', len(src_tokens) - 1)
        print('len(tgt_tokens):', len(tgt_tokens))
        print('src_line:',src_line)
        print('src_tokens:',src_tokens)
        print('aligned_results:',aligned_results)
