...
with open(f'{dataset_path}/validset.json', 'r', encoding='utf-8') as json_file,\
    ...
    for entry in data:
        ...
        # 将Unicode转为中文文本
        # source_text = source_text.encode().decode('ut')
        # target_text = target_text.encode().decode('unicode_escape')
...
with open(f'{dataset_path}//testset.json', 'r', encoding='utf-8') as json_file,\
    ...
    for entry in data:
        ...
        source_text = source_text[:-1] + ' ' + source_text[-1]
        target_text = target_text[:-1] + ' ' + target_text[-1]
