...
class TransformerLayer(nn.Module):
    ...
    def __init__(self, embed_dim, ff_embed_dim, num_heads, dropout, with_external=False, weights_dropout=True):
        ...
        # with_external为True时, 则多出一层encoder-decoder attention layer
    ...
    def forward(self, x, kv=None,
        ...
        x = self.fc2(x)
        x = F.dropout(x, p=self.dropout, training=self.training)
        # print('before ff_layer_norm', x)
        x = self.ff_layer_norm(residual + x)
        # print('after ff_layer_norm', x)
        # exit()
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=None, need_weights=False):
        ...
        if qkv_same:
            # self-attention
            q, k, v = self.in_proj_qkv(query)   # [seq_len, batch_size, embedding_dim]
        elif kv_same:
        ...
        attn_weights = torch.bmm(q, k.transpose(1, 2))      # [batch_size * heads, tgt_len, src_len]
...
class LearnedPositionalEmbedding(nn.Module):
    ...
    def forward(self, input, offset=0):
        """Input is expected to be of size [seq_len x batch_size]."""
        seq_len, bsz = input.size()  # [seq_len, batch_size] = [50, 32]
        positions = (offset + torch.arange(seq_len))

        if self.device: positions = positions.cuda(self.device)

        res = self.weights(positions).unsqueeze(1).expand(-1, bsz, -1)
        # [seq_len, batch_size, embedding_dim] = [50, 32, 300]
        return res
