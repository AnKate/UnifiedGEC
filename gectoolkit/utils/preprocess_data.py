...
from gectoolkit.utils.helpers import (SEQ_DELIMITERS, encode_verb_form,
                                             apply_reverse_transformation, START_TOKEN, write_lines)
...
def apply_merge_or_swap(source_tokens, target_words, shift_idx):
    """
...
def align_sequences(args):
    ...
    for idx, diff in enumerate(diffs):
        ...
        if tag == 'equal':
            ...
            edits = apply_merge_or_swap(source_part, target_part, shift_idx=i1)
            if edits:
...
def add_labels_to_the_tokens(source_tokens, labels, delimeters=SEQ_DELIMITERS):
...
def convert_data_to_vocab(source_lines, target_lines, vocab_path, min_count, save_vocab, worker_num=8):
    """
    为输入数据分配编辑标签, 生成词表
    """
    tagged = []
    dic = defaultdict(int)  # 统计一下当前数据集里的编辑label及其出现次数，过滤掉出现次数过少的
    print(f"The size of raw dataset is {len(source_lines)}")
    with Pool(worker_num) as pool:
        for aligned_sent, align_labels in pool.imap(align_sequences, tqdm(zip(source_lines, target_lines)), chunksize=8):
...
def convert_tagged_line(line, delimeters=SEQ_DELIMITERS):
